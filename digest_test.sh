...
# Unit tests for digest.par

# Trick to chase the symlink before the docker build.
cp digest.par digest2.par
...
# Test pushing an image by just invoking the digest
function test_digest() {
  ...
  digest.par --tarball="${random_image}" --output-digest="${output_file}"
